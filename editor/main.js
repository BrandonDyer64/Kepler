...
exports.openProjectDialog = (callbackProgress, callbackFail) => {
  ...
  dialog.showOpenDialog(
    projectWindow,
    {
      defaultPath: homedir + "/KeplerProjects",
      properties: ["openFile"]
    },
    paths => {
      console.log(paths);
...
module.exports.createProject = name => {
  const dir = homedir + "/KeplerProjects/" + name.replace(/ /g, "");
  if (!fs.existsSync(homedir + "/KeplerProjects"))
    fs.mkdirSync(homedir + "/KeplerProjects");
  if (fs.existsSync(dir)) {
    return { error: true, message: "Project already exists" };
  }
  fs.mkdirSync(dir);
  fs.writeFileSync(
    dir + "/kepler.json",
